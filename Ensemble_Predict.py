...
for model in models:
    ...
    for p in test.Patient.unique():
        ...
        for i in ldir:
            if int(i[:-4]) / len(ldir) < 0.8 and int(i[:-4]) / len(ldir) > 0.15:
                x.append(
                    get_img(
                        f"your_path/osic-pulmonary-fibrosis-progression/test/{p}/{i}"
                    )
                )
        ...
        A_test[p] = a
        B_test[p] = (
            test.FVC.values[test.Patient == p]
            - a * test.Weeks.values[test.Patient == p]
        )
    ...
    for k in sub.Patient_Week.values:
        ...
        sub.loc[sub.Patient_Week == k, "Confidence"] = P_test[p] - A_test[p] * abs(
            WEEK[p] - w
        )
...
def mloss(_lambda):
    def loss(y_true, y_pred):
        return _lambda * new_asy_qloss(y_true, y_pred) + (1 - _lambda) * score(
            y_true, y_pred
        )
...
for i in range(len(test_a)):
    sub.loc[
        sub["Patient_Week"] == test_a.Patient[i] + "_" + str(test_a.Weeks[i]), "FVC"
    ] = test_a.FVC[i]
    sub.loc[
        sub["Patient_Week"] == test_a.Patient[i] + "_" + str(test_a.Weeks[i]),
        "Confidence",
    ] = 0.1
...
test = pd.read_csv("your_path/test.csv").rename(
    columns={
        "Weeks": "base_Week",
        "FVC": "base_FVC",
        "Percent": "base_Percent",
...
def run_single_lasso(param, train_df, test_df, folds, features, target, fold_num=0):
    ...
    logger.info(
        "fold{} score: {:<8.5f}".format(
            fold_num, np.sqrt(mean_absolute_error(target[val_idx], oof[val_idx]))
        )
    )
...
def run_kfold_lasso(param, train, test, folds, features, target, n_fold=5):
    ...
    for fold_ in range(n_fold):
        ...
        _oof, _predictions = run_single_lasso(
            param, train, test, folds, features, target, fold_num=fold_
        )
        oof += _oof
        predictions += _predictions / n_fold

    logger.info("CV score: {:<8.5f}".format(np.sqrt(mean_absolute_error(target, oof))))
...
oof, predictions = run_kfold_lasso(
    xgb_param, train, test, folds, features, target, n_fold=N_FOLD
)
...
oof, predictions = run_kfold_lasso(
    ridge_param, train, test, folds, features, target, n_fold=N_FOLD
)
...
sub4 = submission.drop(columns=["FVC", "Confidence"]).merge(
    test[["Patient_Week", "FVC_pred", "Confidence"]], on="Patient_Week"
)
...
df3 = elastic_sub.sort_values(by=["Patient_Week"], ascending=True).reset_index(
    drop=True
)
...
df["FVC"] = (
    0.25 * df1["FVC"] + 0.44 * df2["FVC"] + 0.18 * df3["FVC"] + 0.13 * df4["FVC"]
)
df["Confidence"] = (
    0.25 * df1["Confidence"]
    + 0.44 * df2["Confidence"]
